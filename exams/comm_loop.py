...
for seed in SEEDS:
    ...
    for s in ("A", "B", "C"):
        ...
        if os.environ.get("COMM_DECODE") == "drop1sd":          # test 2: answer channel falls below its own pre baseline
...
if os.environ.get("COMM_DECODE") == "drop1sd":
...
out = {"protocol": "comm_protocol.md (predeclared 2026-09-17 03:20 EDT)", "graph": str(GRAPH), "seeds": SEEDS, "c_hz": c_hz,
       "mbon_gain": MBON_GAIN, "thresh": os.environ.get("COMM_THRESH", "scalar -45"), "lr": LR, "epochs": TEACH_EPOCHS, "reps": REPS, "A": A_TYPE, "B": B_TYPE, "C": C_TYPE, "a_hz": A_HZ, "b_hz": B_HZ, "answer": ANS_TYPES, "pooled_avoid_rate": pool, "seeds_B_gt_A": seeds_B_gt_A, "R_drop_seeds": nR, "P_drop_seeds": nP, "L_drop_seeds": nL, "decode": os.environ.get("COMM_DECODE", "avoid2sd"), "criteria": crit, "PASS": all(crit.values()),
